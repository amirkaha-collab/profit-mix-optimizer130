...
──────────────────────────────────────────────
Builds analytical prompts and calls OpenAI for institutional
...
def _get_model_name(default: str = "gpt-4o-mini") -> str:
    try:
        import streamlit as st
...
def _call_openai(
    ...
    if not api_key:
        return "", "לא הוגדר מפתח API. הוסף OPENAI_API_KEY ב-Settings → Secrets."
    ...
    try:
        ...
        if resp.status_code == 200:
            out = _extract_openai_text(resp.json())
            return (out, None) if out else ("", "תגובה ריקה מהמודל.")
        if resp.status_code in (401, 403):
            return "", "מפתח OpenAI לא תקין או שלפרויקט אין הרשאה למודל שנבחר."
        if resp.status_code == 429:
            return "", "חריגה ממגבלת קצב או תקציב. נסה שוב בעוד כמה שניות."
        try:
            err = resp.json().get("error", {}).get("message", "")
        except Exception:
            err = ""
        suffix = f": {err}" if err else ""
        return "", f"שגיאת OpenAI: HTTP {resp.status_code}{suffix}"
...
_SYSTEM = (
    "אתה מנתח אסטרטגיות מוסדיים הכותב בעברית ברורה, מקצועית ונעימה לקריאה. "
    "המטרה שלך היא להפוך נתונים וסטטיסטיקות להבנה פשוטה: מה בולט, מה השתנה, ומה המשמעות האפשרית. "
    "בסס כל טענה על הנתונים שסופקו. "
    "כאשר מדובר בפרשנות ולא בעובדה ישירה, נסח בזהירות במילים כמו 'כנראה', 'עשוי להעיד' או 'יכול ללמד'. "
    "אל תשתמש בז'רגון מיותר, אל תעמיס רשימות ארוכות, ואל תציג נוסח אקדמי או שיווקי. "
    "אל תמציא נתונים. אל תציין שאתה AI. "
    "יחס מט\"ח/חו\"ל הוא אינדיקציה חלקית בלבד למדיניות גידור, ולא מדד מושלם."
)


def _style_instructions(context: Optional[dict]) -> str:
    context = context or {}
    style = str(context.get("ai_style", "לקוח פשוט"))
    free_prompt = str(context.get("ai_free_prompt", "")).strip()

    if style == "יועץ מקצועי":
        style_text = (
            "כתוב בעברית מקצועית אך קריאה. שלב מספרים חשובים מתוך הנתונים, "
            "אבל שמור על זרימה טבעית והימנע מרשימות ארוכות. "
        )
    elif style == "פמילי אופיס":
        style_text = (
            "כתוב בסגנון תמציתי, אלגנטי ובשל, כמו תקציר לוועדת השקעות או פמילי אופיס. "
            "הדגש מיצוב יחסי, עקביות, שינויים אחרונים ומשמעות אסטרטגית. "
        )
    else:
        style_text = (
            "כתוב בשפה פשוטה, סיפורית וברורה ללקוח. פתח במשפט אחד שמסביר מה הכי בולט, "
            "אחר כך תאר מה קרה לאחרונה, וסיים במה זה עשוי ללמד. "
            "אל תשתמש בסימונים כמו σ או ברצפים טכניים ארוכים. "
        )

    if free_prompt:
        style_text += f"המשתמש ביקש להתמקד גם בכך: {free_prompt}. "
    return style_text
...
---
כתוב ניתוח שוק קצר, מסודר וקריא עם הכותרות הבאות בדיוק (##):

## התמונה בקצרה
פסקה קצרה שמסבירה מה הכי בולט כרגע בין הגופים שנבחרו.

## מי בולט ולמה
פסקה או שתיים שמסבירות מי גבוה או נמוך יחסית בחו"ל, מניות, מט"ח ולא-סחיר,
מה השתנה לאחרונה, ואיפה יש גוף חריג לטובה או לרעה.

## מה זה אומר
פסקה מסכמת: האם השוק נראה יותר זהיר או יותר אגרסיבי, מי נראה יציב יותר,
ומי נראה דינמי יותר, ומה זה עשוי ללמד חוסך או יועץ."""
...
כתוב ניתוח מיקוד פשוט, זורם ומבוסס נתונים עם הכותרות הבאות בדיוק (##):

## מה בולט במסלול
פסקה קצרה מאוד שמסבירה מה הכי בולט במסלול של הגוף הזה מול עמיתיו,
תוך שימוש במספרים החשובים בלבד.

## מה קרה לאחרונה
פסקה אחת שמסבירה אילו רכיבים עלו או ירדו ב-3–12 החודשים האחרונים,
האם יש כאן שינוי כיוון, והאם זה נראה מהלך חריג או די שגרתי מול המתחרים.

## מה זה עשוי ללמד
פסקה אחת או שתיים, בשפה אנושית וברורה, על סגנון הניהול של הגוף:
האם הוא נראה יציב או דינמי, האם הוא נוטה ליותר חו"ל, מניות או לא-סחיר,
ומה יחס מט"ח/חו"ל עשוי לרמוז לגבי גידור.
חשוב להדגיש הסתייגויות כשמדובר בפרשנות.

## למי זה יכול להתאים
פסקה מסכמת, לא המלצה אישית, שמסבירה בקצרה לאיזה סוג לקוח או יועץ
התמונה הזו יכולה להתאים יותר: מי שמחפש יציבות יחסית, מי שמוכן ליותר תזוזה,
ומי מוכן לחשיפה גבוהה יותר לחו"ל או לנכסים לא סחירים."""
...
---
כתוב השוואה קצרה, ברורה וקריאה עם הכותרות הבאות בדיוק (##):

## התמונה בקצרה
פסקה קצרה שמסבירה מה ההבדל המרכזי בין שני הגופים.

## איפה כל אחד בולט
פסקה או שתיים: מי גבוה יותר בחו"ל, מניות, מט"ח או לא-סחיר,
מי זז יותר, ומי נראה יציב יותר.

## מה זה עשוי לומר על סגנון הניהול
פסקה אחת שמתרגמת את הנתונים לסיפור ניהולי: מי יותר דינמי,
מי כנראה מגדר יותר, ומי נראה עקבי יותר לאורך זמן.

## למי כל אחד עשוי להתאים
פסקה מסכמת, לא כהמלצה אישית, שמבהירה באיזה סוג העדפה
כל אחד עשוי להתאים יותר: יציבות, גמישות, חשיפה לחו"ל, או נכונות לקחת יותר סיכון."""
...
def run_ai_analysis(display_df: pd.DataFrame, context: dict) -> AnalysisResult:
    ...
    text, err = _call_openai(prompt, system=_SYSTEM + " " + _style_instructions(context), max_tokens=1800)
...
def run_focused_analysis(
    ...
    text, err = _call_openai(prompt, system=_SYSTEM + " " + _style_instructions(context), max_tokens=2200)
    result = AnalysisResult(raw_text=text, error=err)
    if text:
        result.parse_sections()
    return result
