...
def _render_api_key_input():
    """Check OpenAI key from secrets/env and show compact status."""
    import os
    has_secret = False
    model_name = os.getenv("OPENAI_MODEL", "gpt-4o-mini")
    try:
        if hasattr(st, "secrets") and "OPENAI_API_KEY" in st.secrets:
            has_secret = bool(str(st.secrets["OPENAI_API_KEY"]).strip())
        if hasattr(st, "secrets") and "OPENAI_MODEL" in st.secrets:
            model_name = str(st.secrets["OPENAI_MODEL"])
    except Exception:
        pass
    if not has_secret:
        has_secret = bool(os.getenv("OPENAI_API_KEY", "").strip())

    if has_secret:
        st.success(f"✅ חיבור OpenAI פעיל דרך OPENAI_API_KEY | מודל: {model_name}", icon="🔑")
        return True

    st.warning("כדי להפעיל ניתוח AI יש להוסיף OPENAI_API_KEY ב-Settings → Secrets.")
    return False
...
<div style='background:{bg};border:2px solid {brd};border-radius:10px;
  ...
  {label}
    ...
    pref1, pref2 = st.columns([1, 2])
    with pref1:
        ai_style = st.selectbox(
            "סגנון כתיבה",
            options=["לקוח פשוט", "יועץ מקצועי", "פמילי אופיס"],
            index=0,
            key="isa_ai_style",
            help="איך לנסח את התשובה: פשוט ללקוח, מקצועי ליועץ או תמציתי בסגנון פמילי אופיס",
        )
    with pref2:
        ai_free_prompt = st.text_area(
            "שאלה או דגש חופשי ל-AI",
            value=st.session_state.get("isa_ai_free_prompt", ""),
            key="isa_ai_free_prompt",
            height=80,
            placeholder="למשל: התמקד בעיקר בשינויים האחרונים, בגידור מט\"ח, ובשאלה עד כמה הגוף דינמי ביחס לאחרים",
            help="אפשר לבקש מה-AI להתמקד בשאלה או בזווית מסוימת בלי קשר לגרפים",
        )
  ...
  <span style='color:#475569;font-size:12px'> — ניתוח יחסי בין כל הגופים שנבחרו: מיצוב, דינמיות, גידור מט"ח, סיכון</span>
        ...
        if cache_key not in st.session_state:
            ...
            if st.button("🌐 הפעל ניתוח שוק", key="isa_market_btn", type="primary",
                ...
                with st.spinner("OpenAI מנתח השוואה בין גופים... (עד 90 שניות)"):
                    try:
                        from institutional_strategy_analysis.ai_analyst import run_ai_analysis
                        market_ctx = {**context, "ai_style": ai_style, "ai_free_prompt": ai_free_prompt}
                        result = run_ai_analysis(display_df, market_ctx)
  ...
  <span style='color:#475569;font-size:12px'> — גוף אחד לעומת כל עמיתיו: מיצוב יחסי, היסטוריה עצמית, סגנון ניהול, גזר דין</span>
        ...
        if cache_key not in st.session_state:
            ...
            if st.button(
                ...
                with st.spinner(f"OpenAI מנתח {focus_mgr} לעומק... (עד 90 שניות)"):
                    try:
                ...
                if st.button("⚖️ הפעל השוואה", key="isa_cmp_btn", type="primary"):
                    ...
                    with st.spinner(f"משווה {mgr_a} מול {mgr_b}... (עד 90 שניות)"):
                        try:
                            ...
                            cmp_ctx = {**context, "ai_style": ai_style, "ai_free_prompt": ai_free_prompt}
                            result = run_comparison_analysis(
                                display_df, mgr_a, trk_a, mgr_b, trk_b, cmp_ctx
                            )
